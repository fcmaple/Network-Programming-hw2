...
int Command::conduct(Pipe& pip){
    ...
    for(auto cmd:command){
        ...
        // cout<<"pip_now_number: "<<pip_now_number<<endl;
        ...
        if(fork()==0){
            if(operation_cursor){ //connect the last command to next command
                ...
                // cout<<"check "<<check<<" command_Line "<<command_line<<endl;
                if(check){
                    // cout<<"number pipe in !\n";
            ...
            if(operation_cursor<operation.size()){
               if(pip_number){
                    ...
                    // cout<<"ordinary set now_pipe"<<pip_now_number<<endl;
                    // if(pip_now_number>=1) pip.dup2_pipe({pipeType::Ordinary,pip_now_number-1,0},pip.pipe_STDIN());
...
Command::Command(){
    ...
    operation_cursor=0;
    //command_line=0;
    // cout<<"init command"<<command_cursor<<" "<<operation_cursor<<endl;
}
